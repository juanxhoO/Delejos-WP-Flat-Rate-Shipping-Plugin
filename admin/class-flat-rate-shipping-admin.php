...
?>
		<div class="custom-shipping-content">
			...
			if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['custom_shipping_submit'])) {
				...
				$custom_rates_table = $wpdb->prefix . 'custom_flat_rate_shippings';
	...
	public function display_cities_by_country()
	{
		...
		if (!empty($results)) {
			...
			foreach ($results as $row) {
				$cityId = esc_html($row['cityId']);
				$country = esc_html($row['countryCode']);
				$price = number_format(doubleval($row['price']), 2); // 2 specifies the number of decimal places			echo($price);
				$id = intval($row['ID']);

				// Fetch cityName corresponding to cityId
				$city_name = $wpdb->get_var($wpdb->prepare("SELECT cityName FROM $cities_table WHERE id = %d", $cityId));
